...
namespace ConferenceVision.iOS {
	...
	/// </summary>
	public class VisionModelInput : NSObject, IMLFeatureProvider
	{
		...
		public VisionModelInput (CVPixelBuffer data)
		{
			if (data == null)
	...
	/// </summary>
	public class VisionModelOutput : NSObject, IMLFeatureProvider
	{
		static readonly NSSet<NSString> featureNames = new NSSet<NSString> (
		...
		public VisionModelOutput (NSDictionary<NSObject, NSNumber> loss, string classLabel)
		{
			if (loss == null)
	...
	/// </summary>
	public class VisionModel : NSObject
	{
		readonly MLModel model;

		public VisionModel ()
		{
			var url = NSBundle.MainBundle.GetUrlForResource ("VisionModel", "mlmodelc");
			NSError err;

			model = MLModel.Create (url, out err);
		}

		VisionModel (MLModel model)
		{
			this.model = model;
		}

		public static VisionModel Create (NSUrl url, out NSError error)
		{
			if (url == null)
			...
			return new VisionModel (model);
		}

		/// <summary>
		/// Make a prediction using the standard interface
		/// </summary>
		/// <param name="input">an instance of VisionModelInput to predict from</param>
		/// <param name="error">If an error occurs, upon return contains an NSError object that describes the problem.</param>
		public VisionModelOutput GetPrediction (VisionModelInput input, out NSError error)
		{
			...
			return new VisionModelOutput (lossValue, classLabelValue);
		}
		...
		/// <param name="error">If an error occurs, upon return contains an NSError object that describes the problem.</param>
		public VisionModelOutput GetPrediction (CVPixelBuffer data, out NSError error)
		{
			var input = new VisionModelInput (data);
